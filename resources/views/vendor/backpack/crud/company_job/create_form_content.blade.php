...
@else
    <div class="card">
        <div class="card-body ">
            <label style="margin-right: 16px">التخصص الرئيسي</label><br>
            <select style="width: 96%;
    margin-right: 16px;" name="category_id" class="form-control select2 category">
                <option value="0" selected disabled>Choose Category</option>

                @foreach(\App\Models\Category::all() as $categoty)
                    <option @if(isset($crud->entry->category_id) && $crud->entry->category_id==$categoty->id) selected
                            @endif value="{{$categoty->id}}">{{$categoty->name_ar}}</option>
            ...
            @if($errors->has('category_id'))
                <div style="margin-right: 16px" class="text-danger">{{ $errors->first('category_id') }}</div>
            @endif
            <label style="margin-right: 16px">التخصص الفرعي</label><br>
            <select style="width: 96%;
    margin-right: 16px;" multiple name="sub_category[]" id="sub_category" class="form-control select2">

            </select>

            @if($errors->has('sub_category'))
                <div style="margin-right: 16px" class="text-danger">{{ $errors->first('sub_category') }}</div>
            @endif
