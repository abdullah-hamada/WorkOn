...
@else
    <div class="card">
        <div class="card-body row">
            ...
            {{--                <option @if(isset($crud->entry->category_id) && $crud->entry->category_id==$categoty->id) selected @endif value="{{$categoty->id}}">{{$categoty->name_ar}}</option>--}}

            {{--            @endforeach--}}
            {{--        </select>--}}

            {{--        <label>التخصص الفرعي</label>--}}
            {{--        <select   multiple name="sub_category[]" id="sub_category" class="form-control select2">--}}
            {{--            @foreach(\App\Models\SubCategory::all() as $sub_category)--}}
            {{--                <option @if($crud->entry->sub_category->has($sub_category)) selected @endif--}}

            {{--                value="{{$sub_category->id}}">{{$sub_category->name_ar}}</option>--}}
