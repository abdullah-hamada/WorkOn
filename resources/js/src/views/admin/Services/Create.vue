  ...
  <v-container fluid>
    <v-card>
      ...
      <v-card-text class="grey lighten-5 mt-3">
        <v-card>
          <v-card-title> بيانات الخدمة</v-card-title>
          <v-card-text>
            <v-form class="multi-col-validation" @submit.prevent="submit">
              ...
              <v-expansion-panels class="mt-3 mb-3" focusable>
                <v-expansion-panel>
                  <v-expansion-panel-header class="text-h5">بيانات اخرى</v-expansion-panel-header>
                  <v-expansion-panel-content>
                    <div class="mb-5 text-center">
                      <div class="image-input">
                        <div
                          :style="
                                  form.preview_description_image != undefined
                                    ? { 'background-image': `url(${ form.preview_description_image})` }
                                    : { 'background-image': `url(${ form.preview_description_image})` }
                                "
                          class="image-input-wrapper"
                        ></div>
                        ...
                        <label class="btn-icon btn-add" data-original-title="Change avatar">
                          ...
                          <input
                            ...
                            name="profile_avatar"
                            type="file"
                            @change="previewDescriptionImage($event)"
                          />

                          <input name="profile_avatar_remove" type="hidden"/>
                        </label>
                      </div>
                      <div v-if="$page.props.errors['description_image']"
                           class="error--text">{{ $page.props.errors['description_image'] }}
                      </div>
                    </div>
                    <v-row>
                      <v-col
                      ...
                      >
                        <v-textarea
                          ...
                          :error-messages="$page.props.errors.paragraph"
                          label="الفقرة"
                          outlined
                      ...
                      >
                        <v-text-field
                          ...
                          :error-messages="$page.props.errors.button_text"
                          dense
                          label="عنوان الزر"
                          outlined
                      ...
                      >
                        <v-text-field
                          ...
                          :error-messages="$page.props.errors.button_url"
                          dense
                          label="رابط الزر"
                          outlined
...
export default {
  ...
  methods: {
    ...
    previewDescriptionImage(event) {
      var input = event.target;
      if (input.files && input.files[0]) {
        var reader = new FileReader();
        reader.onload = (e) => {
          this.form.preview_description_image = e.target.result;
          this.form.description_image = input.files[0];
        };
        reader.readAsDataURL(input.files[0]);
      }
    },
